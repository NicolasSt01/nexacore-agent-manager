...
import { ArrowRight, Facebook, Globe2, Instagram, Lock, MessageCircle, Radio, type LucideIcon } from "lucide-react";
...
export default function ChannelsPage() {
  ...
  return <div className="page"><PageHead eyebrow={t("channels.head.eyebrow")} title={t("channels.head.title")} description={t("channels.head.description")} />
    ...
    <section className="channel-grid"><article className="channel-card channel-card-live"><div className="channel-icon whatsapp"><MessageCircle size={24} /></div><span className="coming live">{t("channels.whatsapp.status")}</span><h3>{t("channels.whatsapp.title")}</h3><p>{t("channels.whatsapp.description")}</p><small className="channel-owner">{selected?.name || t("channels.whatsapp.ownerPlaceholder")}</small>{selected ? <Link className="button primary" href={`/clients/${selected.id}/channels/whatsapp`}>{t("channels.whatsapp.configure")} <ArrowRight size={16} /></Link> : <button className="button disabled" disabled>{t("channels.whatsapp.selectClient")}</button>}</article>{futureChannels.map((channel) => <article className="channel-card" key={channel.key}><div className={`channel-icon ${channel.className}`}><channel.icon size={24} /></div><span className="coming"><Lock size={12} /> {t("channels.future.comingSoon")}</span><h3>{t(`channels.future.${channel.key}.name`)}</h3><p>{t(`channels.future.${channel.key}.description`)}</p><small className="channel-owner">{selected?.name || t("channels.future.ownerPlaceholder")}</small><button className="button disabled" disabled>{t("channels.future.connect")}</button></article>)}</section>
