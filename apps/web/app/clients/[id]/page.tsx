...
export default function ClientDetailPage() {
  ...
  const load = () => api<Client>(`/clients/${id}`).then((row) => {
    setClient(row);
    setBilling({
      billing_mode: row.billing_mode,
      monthly_fee_mxn: row.monthly_fee_mxn,
  ...
  async function saveDetails(event: FormEvent<HTMLFormElement>) {
    ...
    try { setClient(await api<Client>(`/clients/${id}`, { method: "PATCH", body: JSON.stringify({ name: data.get("name"), industry: data.get("industry"), description: data.get("description"), general_context: data.get("general_context"), is_active: data.get("is_active") === "on", ...(billing ?? {}) }) })); toast.success(t("clients.detail.detailsSaved")); }
